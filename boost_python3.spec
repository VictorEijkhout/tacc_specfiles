#
# ./build_rpm.sh -i19 -j19_9 --python=3_9_2 -l boost-mpi_python3.spec
# ./build_rpm.sh -g91 -j19_9 --python=3_9_2 -l boost-mpi_python3.spec
# NOT YET: WAITING FOR PYTHON3 ./build_rpm.sh --gcc=122 -j19_9 --python=3_9_2 -l boost-mpi_python3.spec
...
#------------------------
  ...
  if [ -d ${TACC_PYTHON_INC}/python${TACC_PYTHON_VER} ] ; then 
    TACC_PYTHON_H_INC=${TACC_PYTHON_INC}/python${TACC_PYTHON_VER} 
  elif [ -d ${TACC_PYTHON_INC}/python${TACC_PYTHON_VER}m ] ; then 
    TACC_PYTHON_H_INC=${TACC_PYTHON_INC}/python${TACC_PYTHON_VER}m 
  else 
    echo "Can not set location of python heaer files" && exit 1
  fi
  ...
  echo "using python : ${TACC_PYTHON_VER} : ${PYTHON_EXE} : ${TACC_PYTHON_H_INC} :  ${TACC_PYTHON_LIB} ;" >> tools/build/example/user-config.jam
  echo "using python : ${TACC_PYTHON_VER} : ${PYTHON_EXE} : ${TACC_PYTHON_H_INC} :  ${TACC_PYTHON_LIB} ;" >> user-config.jam
  ./bootstrap.sh \
        --with-python=${PYTHON_EXE}  --with-python-version=${TACC_PYTHON_VER} --with-python-root=${TACC_PYTHON_DIR} \
        --prefix=%{INSTALL_DIR} ${CONFIGURE_FLAGS}
  ./b2 -j 24  include="${TACC_PYTHON_H_INC}" \
        --prefix=%{PYTHON_INSTALL_DIR} $EXTRA cxxflags="%{TACC_OPT}" cflags="%{TACC_OPT}" linkflags="%{TACC_OPT}" \
        install
...
* Thu Jul 14 2022 <eijkhout@tacc.utexas.edu>
- release 2 : victor taking over, trying to make python portable
